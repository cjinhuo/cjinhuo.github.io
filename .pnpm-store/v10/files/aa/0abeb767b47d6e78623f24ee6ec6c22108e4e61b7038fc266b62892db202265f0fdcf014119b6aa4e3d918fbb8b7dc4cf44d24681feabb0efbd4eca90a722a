...
function normalizeURL(elem, hostname, lastmodDateOnly = false) {
        ...
        smi.video = smiLoose.video.map((video) => {
            ...
            if (video.rating !== undefined) {
                if (typeof video.rating === 'string') {
                    ...
                    if (Number.isNaN(parsedRating)) {
                        throw new Error(`Invalid video rating "${video.rating}" for URL "${smi.url}": must be a valid number`);
                    }
            ...
            if (typeof video.view_count === 'string') {
                ...
                if (Number.isNaN(parsedViewCount)) {
                    throw new Error(`Invalid video view_count "${video.view_count}" for URL "${smi.url}": must be a valid number`);
                }
                if (parsedViewCount < 0) {
                    throw new Error(`Invalid video view_count "${video.view_count}" for URL "${smi.url}": cannot be negative`);
                }
    ...
    if (smiLoose.lastmodfile) {
        ...
        // Validate the date is valid
        if (Number.isNaN(lastmodDate.getTime())) {
            throw new Error(`Invalid lastmodISO "${smiLoose.lastmodISO}" for URL "${smi.url}": must be a valid date string`);
        }
        smi.lastmod = lastmodDate.toISOString();
    }
        ...
        // Validate the date is valid
        if (Number.isNaN(lastmodDate.getTime())) {
            throw new Error(`Invalid lastmod "${smiLoose.lastmod}" for URL "${smi.url}": must be a valid date string`);
        }
        smi.lastmod = lastmodDate.toISOString();
    }
