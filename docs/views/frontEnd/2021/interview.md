...
::: tip 履历
本人19年双非普通本科毕业，2020年从小厂跳到中厂，2021年从中厂跳到大厂，借着上一年自研了前端监控系统的经历认真写了简历，目前监控SDK已开源：[mitojs](https://github.com/mitojs/mitojs)（star近1k，已有部分开发者正使用该SDK在构建字节自己的监控平台），列出2021年6月面过的公司以及能记住的一些题目。
:::
...
4. 手写代码：实现一个函数，监听浏览器所有的事件（不可枚举）并上报该事件名称
5. 有写过plugin吗，说说webpack的plugin是怎么获取hook入参的？
6. 手写代码：用react hook实现计时器
7. 手写代码：实现类似webpack plugin的管道函数（具体题目忘记了）
