...
export default function BlogCard({ url, frontmatter }: MarkdownInstance<BlogType>) {
  ...
  return (
    <div className='w-full h-auto mb-6'>
      <a href={url} className='text-3xl text-skin-neutral-1 font-semibold block mb-2 hover:underline'>
        {title}
      </a>
      <time className='font-semibold text-skin-neutral-2'>{dayjs(pubDate).format('MMMM DD,YYYY')}</time>
      {author && (
