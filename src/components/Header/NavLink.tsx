    ...
    <>
      {NavList.map(item => {
        ...
        if (item.label === 'Github') {
          label = <IconPark className='hover:text-skin-neutral-1 transition-colors' name={NameTypes.github}></IconPark>
        }
        return (
          <a
            key={item.href}
            ...
            href={item.href}
            className={`hover:text-skin-neutral-1 transition-colors text-base ${textClassName}`}>
            {label || item.label}
          </a>
        )
      })}
